...
<div class="azr-filter-bar azr-no-print">
    <form method="get" action="/tax/output" style="display:flex;flex-wrap:wrap;gap:10px;align-items:flex-end;flex:1;">
        ...
        <a href="/tax/output" class="azr-btn azr-btn-outline">Reset</a>
    </form>
</div>

<div class="azr-card">
    <div class="azr-card-header">
        <h2 class="azr-card-title">Pajak Keluaran <span style="color:var(--azr-gray-600);font-weight:400;">(<?= (int) $total ?> baris)</span></h2>
        <div class="azr-no-print" style="display:flex;gap:8px;">
            <a href="/tax/output-export.php?<?= Response::e(http_build_query($_GET)) ?>" class="azr-btn azr-btn-outline azr-btn-sm">Export CSV</a>
    ...
    <div class="azr-table-wrap">
        <table class="azr-table">
            <thead><tr><th>Invoice</th><th>Tanggal</th><th>Customer</th><th>Toko</th><th>Jenis Pajak</th><th>DPP</th><th>Tarif</th><th>Jumlah Pajak</th><th>No. Faktur</th><th>Status</th><?php if ($canManage): ?><th></th><?php endif; ?></tr></thead>
            <tbody>
            <?php foreach ($rows as $r): ?>
                <tr>
                    <td><a href="/sales/show.php?id=<?= (int) $r['transaction_id'] ?>"><?= Response::e($r['invoice_no']) ?></a></td>
                    ...
                    <td class="azr-no-print">
                        <button type="button" class="azr-btn azr-btn-outline azr-btn-sm"
                                data-azr-modal-open="invModal_sale_<?= (int) $r['transaction_id'] ?>">Faktur</button>
                    </td>
                    <?php endif; ?>
                </tr>
            <?php endforeach; ?>
    ...
    <div class="azr-pagination azr-no-print">
        <?php $qs = $_GET; for ($i = 1; $i <= $totalPages; $i++): $qs['page'] = $i; ?>
            <a href="/tax/output?<?= Response::e(http_build_query($qs)) ?>" class="<?= $i === $page ? 'active' : '' ?>"><?= $i ?></a>
        <?php endfor; ?>
    </div>
    <?php endif; ?>
</div>
...
<div class="azr-modal-backdrop" id="invModal_sale_<?= (int) $r['transaction_id'] ?>">
    <div class="azr-modal">
        <h3 class="azr-modal-title">Nomor Faktur Pajak - <?= Response::e($r['invoice_no']) ?></h3>
        <p class="azr-help-text">Nomor ini adalah catatan referensi internal AZARED, bukan nomor resmi dari sistem e-Faktur DJP.</p>
