...
<div class="azr-card" style="max-width:720px;">
    <div class="azr-card-header">
        <h2 class="azr-card-title">Edit Pengguna</h2>
    </div>

    <?php if (!empty($errors)): ?>
    <div class="azr-alert azr-alert-error">Periksa kembali form di bawah ini — terdapat kesalahan input.</div>
    <?php endif; ?>

    <?php if ($isSelf): ?>
    <div class="azr-alert azr-alert-info">
        Anda sedang mengedit akun Anda sendiri. Untuk mencegah eskalasi hak akses, <strong>role, akses toko, dan status akun tidak dapat diubah sendiri</strong> — minta pengguna Admin/Manager lain untuk melakukannya.
    </div>
    <?php endif; ?>

    <form action="/users/update.php?id=<?= (int) $old['id'] ?>" method="post" novalidate>
        <?= Csrf::field() ?>
        ...
        <div style="display:flex;gap:16px;">
            <div class="azr-form-group" style="flex:1;">
                ...
                <select class="azr-select" id="role_id" name="role_id" <?= $isSelf ? 'disabled' : '' ?> required>
                ...
                <?php if ($isSelf): ?>
                    <input type="hidden" name="role_id" value="<?= azrOld($old, 'role_id') ?>">
                <?php endif; ?>
            ...
            <div class="azr-form-group" style="flex:1;">
                ...
                <select class="azr-select" id="status" name="status" <?= $isSelf ? 'disabled' : '' ?>>
                ...
                <?php if ($isSelf): ?>
                    <input type="hidden" name="status" value="<?= azrOld($old, 'status') ?>">
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="azr-form-group">
            ...
            <?php foreach ($stores as $store): ?>
                <label style="display:flex;align-items:center;gap:8px;margin-bottom:6px;font-weight:400;">
                    <input type="checkbox" name="store_ids[]" value="<?= (int) $store['id'] ?>" <?= $isSelf ? 'disabled' : '' ?>
                        <?= in_array((string) $store['id'], (array) $oldStoreIds, true) ? 'checked' : '' ?>>
                    <?= Response::e($store['name']) ?> (<?= Response::e($store['code']) ?>)
                </label>
            <?php endforeach; ?>
            <?php if ($isSelf): ?>
                <?php foreach ((array) $oldStoreIds as $sid): ?>
        ...
        <div style="display:flex;gap:10px;margin-top:20px;">
            <button type="submit" class="azr-btn azr-btn-primary">Simpan Perubahan</button>
